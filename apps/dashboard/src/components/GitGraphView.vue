...
const svgWidth = computed(() => labelX.value + 600)
...
function targetLane(afterLanes: (string | null)[], fromLane: number, nextRow: number): number {
  ...
  // Find if this lane continues at same position
  return fromLane
}
...
// Compute x-offsets for ref badges per row (need to measure text width approximately)
