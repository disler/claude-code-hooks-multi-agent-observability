/**
 * Task Completed Hook (TypeScript port)
 * Tracks task completion events. Logs the full input data
 * to the session log directory.
 *
 * Usage: bun .claude/hooks-ts/task_completed.ts
 */
...
async function main() {
  try {
    ...
    const logPath = join(logDir, "task_completed.json");
