/**
 * Subagent Stop Hook (TypeScript port)
 * Tracks subagent lifecycle stop events. Logs agent_id, agent_type,
 * agent_transcript_path, and optionally exports the chat transcript.
 * NO TTS -- that is Python-only.
 *
 * Usage: bun .claude/hooks-ts/subagent_stop.ts [--chat]
 */
...
async function main() {
  try {
    ...
    const agentTranscriptPath =
      (inputData.agent_transcript_path as string) ?? "";
    ...
    const logEntry = {
      ...
      hook_event_name:
        (inputData.hook_event_name as string) ?? "SubagentStop",
