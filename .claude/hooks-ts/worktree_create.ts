/**
 * Worktree Create Hook (TypeScript port)
 * Tracks worktree creation events when agent worktree isolation
 * creates a new worktree. Logs the full input data to the session
 * log directory.
 *
 * Usage: bun .claude/hooks-ts/worktree_create.ts
 */
...
async function main() {
  try {
    ...
    const logPath = join(logDir, "worktree_create.json");
