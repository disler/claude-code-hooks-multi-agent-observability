/**
 * Worktree Remove Hook (TypeScript port)
 * Tracks worktree removal events when agent worktree isolation
 * removes a worktree. Logs the full input data to the session
 * log directory.
 *
 * Usage: bun .claude/hooks-ts/worktree_remove.ts
 */
...
async function main() {
  try {
    ...
    const logPath = join(logDir, "worktree_remove.json");
